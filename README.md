...
社内エンジニア向けの10分発表と、5分の質疑を想定した公開資料です。自宅のProxmox上に置く作業環境と情報基盤、Codexから使うクラウドモデル、GitHubの関係を先に見せます。人間が段取りを中継していた頃と独自Harnessの撤去を挟み、「古い公開スライド生成セットを現行13枚の設計に揃えて公開する」という説明用の依頼を追います。途中の判断と失敗は説明用に再構成したもので、実行記録ではありません。
